...
**Customer Churn Prediction** — Advanced Apex Project for M.Sc. DSAI (Trimester 2). End-to-end ML project using Python, Jupyter Notebooks, and scikit-learn. **Not** an algorithm-study workspace or production system.

## Non-negotiable: Swamy only

This repository is **Swamy PKV's personal project work**. It is **not** maintained for other learners, employers, or the public. Do **not** reword `README.md` or docs to imply a general audience unless Swamy explicitly asks.

## Project layout

Follows the structure in `docs/01-repository-structure.md`:

- `data/raw/` — immutable source datasets (read-only, never modify)
...
Bundled on-demand procedures live under `.github/skills/` (mirrored at `.cursor/skills/`). They use YAML frontmatter plus a Markdown body so agents can match tasks without loading everything up front. How this fits `CLAUDE.md`, rules, and MCP: **`docs/agent-skills.md`**.

## Agent subagents (Cursor)

**Custom subagents** live under **`.cursor/agents/`** (YAML frontmatter + instructions). Cursor uses them for delegated tasks with a fresh context. The same files are **mirrored** at **`.github/agents/`** for visibility in Git; keep both trees identical when editing.

- **Index and how this fits `CLAUDE.md`:** **`docs/agent-subagents.md`**
- **Invocation:** natural language ("use the churn-ci-verify subagent") or `/churn-ci-verify` when supported.
...
| **Playbooks** | **`.github/skills/`** (mirrored **`.cursor/skills/`**), **`.cursor/agents/`** (mirrored **`.github/agents/`**) | How to run CI, audit notebooks, etc. |
...
- **Project scope**: End-to-end ML for customer churn. See `.cursor/rules/05_primary-directives.mdc`.
- **Repository structure**: Follow `docs/01-repository-structure.md` and `.cursor/rules/02_repository-structure.mdc`.
- **Scope**: See `.cursor/rules/swamy_personal_learning_only.mdc`.
- **Raw data**: `data/raw/` is read-only. See `.cursor/rules/06_source_material_rules.mdc`.
- **Notebook quality**: See `.cursor/rules/01_educational-content-rules.mdc`.
...
| `docs/01-repository-structure.md` | Canonical repo structure (single source of truth) |
...
| `.cursor/rules/01_educational-content-rules.mdc` | Notebook quality and analysis standards |
...
| `.cursor/rules/06_source_material_rules.mdc` | `data/raw/` — immutable source data |
